...
| `?` | Адреса перехода: переход по первому адресу перехода, если ячейка отмечена, в противном случае переход по второму адресу перехода | Определение того, является ли ячейка, на которую указывает головка, маркированной или нет |
...
- прокрутка ленты (преодоление ограничения количества столбцов дисплея) с полосой прокрутки на месте: стрелками для каждой стороны показывается, есть ли ещё лента
